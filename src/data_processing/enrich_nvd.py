...
for file in NVD.glob("*.json"):
    ...
    # The English NVD description is selected.
    description = ""
    ...
    # The English CWE value is selected.
    cwe = ""
...
with_cvss = sum(
    bool(row.get("cvss_score"))
    for row in rows
)

with_description = sum(
    bool(row.get("nvd_description"))
    for row in rows
)
...
print(f"CVEs with description: {with_description}")
print(
    f"CVEs without description: "
    f"{len(rows) - with_description}"
)